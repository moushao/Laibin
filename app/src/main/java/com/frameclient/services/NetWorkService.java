    ...
    protected void onHandleIntent(Intent arg0) {
        ...
        switch (opera) {
            ...
            case 2: //get resource
            {
                ...
                if (from == 0) {
                    Log.v(TAG, "recv com.frameclient.getresource message uid =" + uid + " resource_id = " +
                            resource_id + " type = " + type + " from = " + from);
                    tcp.sendGetResourceMsg(uid, resource_id, type);
                } else {

                    String stream_server = arg0.getExtras().getString("stream_server");
                    Log.v(TAG, "recv com.frameclient.getresource message uid =" + uid + " resource_id = " +
                            resource_id + " stream_server = " + stream_server + " from = " + from);
